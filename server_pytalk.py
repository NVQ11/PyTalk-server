...
# Giả lập Cơ sở dữ liệu tập trung lưu trên Server (Không lưu cục bộ máy Client)
# Có sẵn một tài khoản test mặc định là: admin mật khẩu: 123
registered_users = {
    "admin": {"password": "123", "email": "admin@pytalk.com"}
}
sid_to_username = {}
rooms_data = {}
...
def generate_room_id():
    while True:
        ...
        if code not in rooms_data: return code
...
def handle_join_room(sid, data):
    ...
    if not username: return {"status": "error", "message": "Chưa xác thực!"}
    if room_id not in rooms_data: return {"status": "error", "message": "Mã phòng không tồn tại hoặc đã đóng!"}
    
    room = rooms_data[room_id]
    if username not in room['users']: room['users'].append(username)
...
def handle_leave_room(sid, data):
    ...
    if room_id in rooms_data and username in rooms_data[room_id]['users']:
        ...
        if not rooms_data[room_id]['users']: del rooms_data[room_id]
        else: update_room_and_lobby(room_id)
...
def update_room_and_lobby(room_id):
    if room_id in rooms_data:
        sio.emit('update_room_info', {'room_name': rooms_data[room_id]['name'], 'room_id': room_id, 'users': rooms_data[room_id]['users']}, room=room_id)
    broadcast_rooms_list()
    ...
    if username:
        ...
        for rid, room in rooms_data.items():
            if username in room['users']:
                ...
                if not room['users']: empty_rooms.append(rid)
                else: sio.emit('update_room_info', {'room_name': room['name'], 'room_id': rid, 'users': room['users']}, room=rid)
        for rid in empty_rooms: del rooms_data[rid]
